...
class DataPrepper:
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        for feat in hit['fields']['_ltrlog'][0]['log_entry']:
            feature_results[feat['name']].append(feat.get('value', 0))
        feature_results["doc_id"].append(doc_id)
        feature_results["query_id"].append(query_id)
        feature_results["sku"].append(doc_id)
        # print(pd.DataFrame(feature_results)[['short_description_phrase_match', 'long_description_phrase_match', 'artist_name_phrase_match']])
        return pd.DataFrame(feature_results).astype({'doc_id': 'int64', 'query_id': 'int64', 'sku': 'int64'})
